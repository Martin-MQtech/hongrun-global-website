...
text = text.replace(
    'Swedish Spring Plates',
    'Proprietary Core Patents'
)
text = text.replace(
    'Premium imported components for 30,000+ hour service life',
    'Self-developed patented valve &amp; pump engineering for 30,000+ hour reliability'
)
...
# 3. Update products-hvs.html for HEPA and Bactericidal Core wording
...
# Replace HEPA with precision micro-filtration
...
# Replace Bactericidal Core (no internal sterilization) with Anti-Corrosion Hydrodynamic Channel
hvs_text = hvs_text.replace(
    '''      <div class="text-center p-6 bg-brand-light rounded-sm border border-slate-200 hover:border-brand-blue transition">
        <i class="fa-solid fa-bacteria text-brand-blue text-3xl mb-4"></i>
        <h3 class="font-bold text-brand-dark font-display text-sm uppercase">Bactericidal Core</h3>
        <p class="text-slate-500 text-xs mt-2">Inner paths treated to prevent biofilm accumulation</p>
      </div>''',
    '''      <div class="text-center p-6 bg-brand-light rounded-sm border border-slate-200 hover:border-brand-blue transition">
        <i class="fa-solid fa-shield-halved text-brand-blue text-3xl mb-4"></i>
        <h3 class="font-bold text-brand-dark font-display text-sm uppercase">Anti-Corrosion Path</h3>
        <p class="text-slate-500 text-xs mt-2">Corrosion-resistant hydrodynamic channels prevent debris &amp; biofilm buildup</p>
      </div>'''
)
...
with open('/Users/martin/Documents/2026 BUSINESS MTRIX /20260810 HR TECH WEBSITE/hrtech/products-hvs.html', 'w', encoding='utf-8') as f:
    f.write(hvs_text)
print("products-hvs.html updated successfully!")
